...
public class FrmCrudEmpleado extends JInternalFrame
	...
	private JLabel lblMantenimientoEmpleado;
	...
	public FrmCrudEmpleado() {
		...
		cboPais.setModel(new DefaultComboBoxModel<String>(new String[] { "[Seleccione]", "Per\u00F9", "Venezuela",
		...
		table.setModel(new DefaultTableModel(new Object[][] {}, new String[] { "C\u00F3digo", "Nombres", "Apellidos",
				"DNI", "Correo", "Fecha de nacimiento", "Pa\u00EDs" }));
		// alineación
		...
		lblMantenimientoEmpleado = new JLabel("Mantenimiento Empleado");
		lblMantenimientoEmpleado.setHorizontalAlignment(SwingConstants.CENTER);
		lblMantenimientoEmpleado.setFont(new Font("Sylfaen", Font.BOLD, 24));
		lblMantenimientoEmpleado.setBounds(10, 10, 868, 37);
	...
	private void lista() {
		EmpleadoModel almd = new EmpleadoModel();
		List<Empleado> lista = almd.listaAlumno();
		...
		for (Empleado x : lista) {
			fila = new Object[] { x.getIdAlumno(), x.getNombre(), x.getApellido(), x.getDni(), x.getCorreo(),
					x.getFechanac(), x.getPais(), x.getFechareg() };
			dtm.addRow(fila);
		}
	}
	...
	private void agrega() {
		...
		if (!name.matches(Validaciones.TEXTO)) {
			...
			EmpleadoModel am = new EmpleadoModel();
	...
	private void busca() {
		...
		System.out.println(idSeleccionado + " - " + nombre + " - " + apellido + " - " + dni + " - " + correo + " - "
				+ fechanac + " - " + pais + " - ");
